...
| `stage-10-review` | The definition model, reviewed | what a phase admits, what order a definition runs in, and what a failed or repeated announcement means are each written down and tested |
| `stage-11-review` | Foreign code and interrupted rollbacks | no application callback can decide a saga's fate, and a rollback that is interrupted finishes as what it started as |
| `stage-12-tracer` | What one saga did, seen from outside | a per-saga event hook exists at zero cost to the row, and the surface question is answered by evidence — or dropped by a kill criterion |
...
## What one saga did — opened 2026-09-24

The last three review rounds found their defects by reading code and by doubles that count calls;
nothing in the library would have shown any of them in a running system. The first idea was a
visual tracer on Compose for Desktop.
[research-petich-tracer.md](docs/research/research-petich-tracer.md) turns it the other way up: the
**hook** is the work and has value with no UI, and each surface — waterfall, page, window — is opened
only by the previous one's red, with kill criteria declared before the work.

Two things about this stage that the items alone do not say:

* **The brief was drafted a review round early.** Re-read at `f47828e`, twelve of its fourteen
  claims needed a correction and three were wrong; its three fixtures were two fixed defects and one
  kept on purpose. Each fixture is now a **pair** of runs the trace must tell apart, both
  reproducible today, and the re-read found one route B-52 left open —
  [B-59](docs/backlog/B-59-an-announcement-s-own-timeout-still-rolls-back.md) — filed as a defect of
  its own rather than kept as a fixture.
* **[B-60](docs/backlog/B-60-a-logging-sink-in-konekt-and-shashki.md) closes on a date, not a
  merge.** Its two weeks are kill criterion 2; B-61…B-63 are `question` until that and the
  following gates say otherwise.
...
| [B-63](docs/backlog/B-63-the-in-process-window.md) `[?]` | Does a Compose window that a test opens show a fault-injection scenario a headless run cannot? | P3 | M | B-62 |
| [B-64](docs/backlog/B-64-chain-unavailable-cannot-happen-any-more.md) `[~]` | onChainUnavailable counts a failure the definition model cannot produce | P3 | XS | - |
...
**Foreign code and interrupted rollbacks**

- [B-51](docs/backlog/B-51-failed-without-being-undone.md) `[x]` - A storage error after an effect writes FAILED and undoes nothing
- [B-52](docs/backlog/B-52-foreign-code-inside-the-catch.md) `[x]` - A hung announcement rolls the saga back, and a handler that throws decides its fate
- [B-53](docs/backlog/B-53-resuspend-remembers-neither-half.md) `[x]` - A parked cascade is not undone, and its phase is not remembered
- [B-54](docs/backlog/B-54-what-a-rollback-will-end-as.md) `[x]` - A resumed rollback forgets it was a refusal, and can drag a terminal saga back
- [B-55](docs/backlog/B-55-the-sweeper-counts-a-refusal-as-a-rescue.md) `[x]` - The sweeper counts a refusal as a rescue, and one failure blocks both queues
- [B-56](docs/backlog/B-56-d14-is-wrong-about-the-stuck-queue.md) `[x]` - D14 says nothing is written, and on the stuck queue something is
- [B-57](docs/backlog/B-57-the-constructor-tail-and-two-leaks.md) `[x]` - The engine's constructor grows a tail, and a reason string reaches the outbox

**What one saga did, seen from outside**

- [B-58](docs/backlog/B-58-a-per-saga-event-hook.md) `[x]` - Nothing reports what one saga did, only how many of a type did it
- [B-59](docs/backlog/B-59-an-announcement-s-own-timeout-still-rolls-back.md) `[x]` - An announcement whose own withTimeout fires still rolls a finished saga back
- [B-65](docs/backlog/B-65-the-engine-never-writes-processing.md) `[x]` - The engine never writes PROCESSING, so a saga that dies mid-pass is found by neither queue
- [B-66](docs/backlog/B-66-an-expiry-forgets-the-member-a-resume-died-in.md) `[x]` - A resume that dies in its first member is later expired without that member
