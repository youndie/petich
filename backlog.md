...
**One item is open, and it was opened by the release week rather than closed by it.**
...
oddity of one container — which was the wrong call, and B-09 now says so at the point where it was
made. A flake that reaches the gate is work, and the item refuses the three ways of making the
report stop instead of the defect.
...
|---|---|---|---|---|
| [B-17](docs/backlog/B-17-native-store-test-flake.md) `[~]` | The native store's concurrency test fails sometimes, with an I/O error from the driver | P1 | S/M | - |
