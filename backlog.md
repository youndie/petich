...
|---|---|---|---|---|
| [B-39](docs/backlog/B-39-a-phase-that-admits-an-acting-member.md) `[~]` | AUTHORIZATION admits an acting member, so the phase stops meaning anything | P1 | M | - |
| [B-40](docs/backlog/B-40-declaration-order-is-not-run-order.md) `[ ]` | A definition can read in one order and run in another | P1 | S | - |
