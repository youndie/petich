...
@Serializable
data class TestPayload(val data: String) : PetichPayload()
...
class TestRepository : PetichRepository {
    ...
    override suspend fun saveOrGet(petich: Petich): Petich =
        petiches.putIfAbsent(petich.id, petich) ?: petich
...
class ProceedInterceptor : PetichInterceptor<TestPayload> {
    ...
    override suspend fun intercept(
        ...
        return InterceptorResult.Proceed(TestEnrichedPayload(processed = true))
    }

    override suspend fun compensate(petich: Petich, payload: TestPayload) {}
}

class RejectInterceptor(private val reason: String) : PetichInterceptor<TestPayload> {
...
class SuspendInterceptor : PetichInterceptor<TestPayload> {
    ...
    override suspend fun intercept(
        ...
        return InterceptorResult.Suspend("SMS_OTP", TestEnrichedPayload(otpCode = "123456"))
    }
...
class PetichRoutingTest {
    ...
    @Test
    fun testCreatePetichHappyPath() = testApplication {
        configureApp(listOf(ProceedInterceptor()))

        val response = client.post("/api/v1/petiches") {
            contentType(ContentType.Application.Json)
            setBody("""{"id":"p1","type":"test","payload":{"type":"ru.workinprogress.petich.ktor.TestPayload","data":"hello"}}""")
        }
    ...
    @Test
    fun testCreatePetichValidationReject() = testApplication {
        configureApp(listOf(RejectInterceptor("Bad request data")))

        val response = client.post("/api/v1/petiches") {
            contentType(ContentType.Application.Json)
            setBody("""{"id":"p2","type":"test","payload":{"type":"ru.workinprogress.petich.ktor.TestPayload","data":"bad"}}""")
        }
    ...
    @Test
    fun testCreatePetichSuspendReturns202() = testApplication {
        configureApp(listOf(SuspendInterceptor(), ProceedInterceptor()))

        val response = client.post("/api/v1/petiches") {
            contentType(ContentType.Application.Json)
            setBody("""{"id":"p3","type":"test","payload":{"type":"ru.workinprogress.petich.ktor.TestPayload","data":"hello"}}""")
        }
    ...
    @Test
    fun testGetPetichNotFound() = testApplication {
        configureApp(listOf(ProceedInterceptor()))
    ...
    @Test
    fun testGetPetichAfterCreate() = testApplication {
        configureApp(listOf(ProceedInterceptor()))

        client.post("/api/v1/petiches") {
            contentType(ContentType.Application.Json)
            setBody("""{"id":"p4","type":"test","payload":{"type":"ru.workinprogress.petich.ktor.TestPayload","data":"hello"}}""")
        }
    ...
    @Test
    fun testResumeNonexistentPetich() = testApplication {
        configureApp(listOf(ProceedInterceptor()))

        val response = client.post("/api/v1/petiches/nonexistent/resume") {
            contentType(ContentType.Application.Json)
            setBody("""{}""")
        }
    ...
    @Test
    fun testResumeTerminalPetichReturnsConflict() = testApplication {
        configureApp(listOf(ProceedInterceptor()))

        client.post("/api/v1/petiches") {
            contentType(ContentType.Application.Json)
            setBody("""{"id":"p5","type":"test","payload":{"type":"ru.workinprogress.petich.ktor.TestPayload","data":"hello"}}""")
        }

        val response = client.post("/api/v1/petiches/p5/resume") {
            contentType(ContentType.Application.Json)
            setBody("""{}""")
        }
